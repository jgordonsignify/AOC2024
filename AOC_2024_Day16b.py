...
def do_race(grid, x, y, current_score, dx, dy, visited_squares):
    global best_score, best_paths_visited
    position_key = (x, y)  # Only use position coordinates as key, not direction
    end_key = (end_x, end_y)
    ...
    if position_key in best_score and current_score > best_score[position_key] + 1001:  # add 1001 to avoid false positives due to direction change HORRIBLE HACK
        # print(f"Already found better path to {position_key} with score {best_score[position_key]}")
        return
        
    # Update best score for this position
    old_score = best_score.get(position_key, float('inf'))
    ...
    if(x == end_x and y == end_y):
        print(f"Reached end with score: {current_score} old score: {old_score}")
        if current_score == old_score:
            best_paths_visited.append(visited_squares)
        else:
            best_paths_visited = [visited_squares]
        #print(f"Best paths visited: {best_paths_visited}")
        return
...
def main():
    ...
    if end_scores:
        ...
        # Get unique squares visited across all best paths
        unique_squares = set()
        for path in best_paths_visited:
            unique_squares.update(path)
        print(f"Number of unique squares visited in best paths: {len(unique_squares) + 1}") # +1 for start
        print("Unique squares visited:", unique_squares)
        # Draw grid with O's for visited squares
        print("\nGrid with visited squares marked:")
        for y, row in enumerate(grid):
            line = ""
            for x, char in enumerate(row):
                if char == '#':
                    line += '#'
                elif (x,y) in unique_squares:
                    line += 'O'
                else:
                    line += '.'
            print(line)
