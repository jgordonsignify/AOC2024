...
foreach my $a_type (keys %{$antennas}){
    ...
    for(my $i = 0;$i<scalar(@node)-1;$i++){
        for(my $j = $i+1;$j<scalar(@node);$j++){
            ...
            my @new_antinodes = (
                [$node[$j]->[0] + $distance->[0], $node[$j]->[1] + $distance->[1]],
                [$node[$i]->[0] - $distance->[0], $node[$i]->[1] - $distance->[1]]
            );
            foreach my $new_an (@new_antinodes){
                if($new_an->[0] < 0 || $new_an->[0] > $xMax || $new_an->[1] < 0 || $new_an->[1] > $yMax){
                    next;
                }
