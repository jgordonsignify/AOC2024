...
def main():
    ...
    min = 1024 #Line we know works from earlier 
    max = 3450 # end of file
    print(f"Trying {min} lines")
    while (min != max):
        halfway = int((min + max) / 2)
        print(f"Trying {halfway} lines\n")
        grid = read_grid(filename, halfway)
        ...
        print(f"Found scores: {end_scores}, min: {min}, max: {max}")
